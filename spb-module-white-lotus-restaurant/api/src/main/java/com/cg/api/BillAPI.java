...
import com.cg.domain.dto.report.DayToDayReportDTO;
...
public class BillAPI {
    ...
    public ResponseEntity<?> getBillsOfTodayAndPreviousDay() {

        String todayStr = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        String previousDayStr = LocalDate.now().minusDays(1).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));

        List<BillGetTwoDayDTO> listBillToday = iBillService.getBillsNotPaging(todayStr);
        List<BillGetTwoDayDTO> listBillPreviousDay = iBillService.getBillsNotPaging(previousDayStr);
    ...
    @PostMapping
    public ResponseEntity<?> createBill(@Validated @RequestBody BillCreateDTO billCreateDTO, BindingResult bindingResult) {

        if (bindingResult.hasFieldErrors()) {
            return appUtils.mapErrorToResponse(bindingResult);
        }

        Long orderId = billCreateDTO.getOrderId();

        Optional<Order> orderOptional = iOrderService.findById(orderId);

        Order order = orderOptional.get();

        if (order.getStatus() == EBillStatus.PAID) {
            throw new DataInputException("Hóa đơn đã thanh toán.");
        }

        List<Bill> billList = iBillService.findALlByOrderIdAndStatus(orderId, EBillStatus.ORDERING);

        if (billList.size() != 0) {
            throw new DataInputException("Bill đang thanh toán không thể tạo thêm.");
        }

        BillResDTO billResDTO = iBillService.createBillWithOrders(billCreateDTO);
...
//        List<Bill> billList = iBillService.findALlByOrderIdAndStatus(orderId, EBillStatus.PAID);
//
//        if(billList.size() != 0) {
//            throw new DataInputException("Hóa đơn đã được thanh toán.");
//        }

        Optional<Bill> billOptional = iBillService.findByOrderAndStatus(order, EBillStatus.PAID);

        if (billOptional.isPresent()) {
            throw new DataInputException("Hóa đơn đã được thanh toán.");
        }

//        BillPayResDTO billPayResDTO = iBillService.pay(orderId);
